  ...
  if (e && e.parameter && e.parameter.action) {
    ...
    if (action === 'list') {
      return ContentService
        .createTextOutput(JSON.stringify(listRecords()))
        .setMimeType(ContentService.MimeType.JSON);
    } else if (action === 'get') {
      var row = Number(e.parameter.row);
      return ContentService
        .createTextOutput(JSON.stringify(getRecord(row)))
        .setMimeType(ContentService.MimeType.JSON);
    } else if (action === 'pdf') {
      var rowPdf = Number(e.parameter.row);
      return ContentService.createTextOutput(exportRecordPdf(rowPdf));
    }
  }
